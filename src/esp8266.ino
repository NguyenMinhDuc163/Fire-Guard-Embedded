...
const char* ssidList[] = {"TP-Link_70A6", "NguyenDuc"};
const char* passwordList[] = {"24492003", "tumotden8"};

// Cấu hình địa chỉ IP tĩnh
IPAddress local_IP(192, 168, 0, 50);     // Địa chỉ IP tĩnh mong muốn
IPAddress gateway(192, 168, 0, 1);       // Địa chỉ gateway của router
...
//const char* serverUrl = "http://192.168.0.101:3000/api/v1/sensors/data";
const char* serverUrl = "http://20.255.153.62:3000/api/v1/sensors/data";
//const char* serverUrl = "https://iot.nguyenduc.click/api/v1/sensors/data";
...
void sendData(int gasValue, int airValue, int flaValue, bool buzzerStatus) {
  if (WiFi.status() == WL_CONNECTED) {  // Kiểm tra kết nối WiFi
    Serial.println("Bắt đầu gửi dữ liệu đến: " + String(serverUrl));
    
    HTTPClient http;
    // Thêm timeout dài hơn (10 giây)
    http.setTimeout(10000);
    
    // Debug kết nối
    Serial.print("Khởi tạo kết nối HTTP... ");
...
//    http.addHeader("Authorization", "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOjEsImlhdCI6MTc0Mjg5NDQwMywiZXhwIjoxNzQyODk4MDAzfQ.wVO1iKIGZxdBTX6CdnW3aHs0_YC19GeMd7_vxTlskbM");
    ...
    Serial.print("Đang gửi HTTP POST... ");
    int httpResponseCode = http.POST(jsonData);

    if (httpResponseCode > 0) {
      String response = http.getString();
      Serial.println("Thành công! Mã phản hồi: " + String(httpResponseCode));
      Serial.println("Phản hồi: " + response);
    } else {
      Serial.println("Lỗi! Mã phản hồi: " + String(httpResponseCode));
      
      // Debug chi tiết lỗi
      String error = http.errorToString(httpResponseCode);
      if (error.length() > 0) {
        Serial.println("Chi tiết lỗi: " + error);
      }
    }
